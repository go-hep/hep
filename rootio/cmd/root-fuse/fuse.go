...
func (fs *FS) get(name string) (rootio.Object, error) {
	if name == "" {
		return fs.root, nil
	}
	dirs := strings.Split(name, "/")
	var (
		ctx rootio.Directory = fs.root
		obj rootio.Object    = fs.root
		err error
	)
	for i, n := range dirs {
		obj, err = ctx.Get(n)
