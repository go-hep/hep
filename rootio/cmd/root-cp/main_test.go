...
func TestROOTCp(t *testing.T) {
	dir, err := ioutil.TempDir("", "rootio-root-cp-")
	if err != nil {
		t.Fatal(err)
	}
	...
	refs := []rootio.Object{
		rootio.NewObjString("string1"),
		rootio.NewObjString("string2"),
		rootio.NewObjString("string3"),
	}
	keys := []string{
		"key", "key-1", "str-3",
	}
