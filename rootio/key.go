...
func (k *Key) Object() (Object, error) {
	...
	buf, err := k.Bytes()
	if err != nil {
		return nil, fmt.Errorf("rootio: could not load key payload: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("rootio: could not unmarshal key payload: %w", err)
	}
...
func (k *Key) load(buf []byte) ([]byte, error) {
	...
	if k.isCompressed() {
		...
		if err != nil {
			return nil, fmt.Errorf("rootio: could not decompress key payload: %w", err)
		}
		return buf, nil
	}
	start := k.seekkey + int64(k.keylen)
...
func (k *Key) adjust(nsize int32) error {
	...
	if best == nil {
		return fmt.Errorf("rootio: could not find a suitable free segment")
	}
