...
type EventData struct {
	...
	VecI64 []int64     `groot:"StlVecI64"`
	VecU16 []uint16    `groot:"StlVecU16"`
	VecU32 []uint32    `groot:"StlVecU32"`
	VecU64 []uint64    `groot:"StlVecU64"`
	VecF32 []float32   `groot:"StlVecF32"`
	VecF64 []float64   `groot:"StlVecF64"`
	VecStr []string    `groot:"StlVecStr"`
...
func TestEventTree(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{
			name:  "nosplit-xrootd",
			fname: rtests.XrdRemote("testdata/small-evnt-tree-nosplit.root"),
		},
		{
...
func TestSimpleTreeOverHTTP(t *testing.T) {
	...
	f, err := riofs.Open("https://github.com/go-hep/hep/raw/master/rootio/testdata/simple.root")
	if err != nil {
		t.Fatal(err)
	}
	defer f.Close()
...
func TestUprootTrees(t *testing.T) {
	type Data struct {
		N     int32      `groot:"n"`
		B     bool       `groot:"b"`
		Arrb  [3]bool    `groot:"ab"`
		Ab    []bool     `groot:"Ab"`
		I1    int8       `groot:"i1"`
		Arri1 [3]int8    `groot:"ai1"`
		Ai1   []int8     `groot:"Ai1"`
		U1    int8       `groot:"u1"`
		Arru1 [3]int8    `groot:"au1"`
		Au1   []int8     `groot:"Au1"`
		I2    int16      `groot:"i2"`
		Arri2 [3]int16   `groot:"ai2"`
		Ai2   []int16    `groot:"Ai2"`
		U2    int16      `groot:"u2"`
		Arru2 [3]int16   `groot:"au2"`
		Au2   []int16    `groot:"Au2"`
		I4    int32      `groot:"i4"`
		Arri4 [3]int32   `groot:"ai4"`
		Ai4   []int32    `groot:"Ai4"`
		U4    int32      `groot:"u4"`
		Arru4 [3]int32   `groot:"au4"`
		Au4   []int32    `groot:"Au4"`
		I8    int64      `groot:"i8"`
		Arri8 [3]int64   `groot:"ai8"`
		Ai8   []int64    `groot:"Ai8"`
		U8    int64      `groot:"u8"`
		Arru8 [3]int64   `groot:"au8"`
		Au8   []int64    `groot:"Au8"`
		F4    float32    `groot:"f4"`
		Arrf4 [3]float32 `groot:"af4"`
		Af4   []float32  `groot:"Af4"`
		F8    float64    `groot:"f8"`
		Arrf8 [3]float64 `groot:"af8"`
		Af8   []float64  `groot:"Af8"`
		Str   string     `groot:"str"`
	}
