	...
	case opAndAnd:
		...
		case bool:
			...
			case bool:
				return r, nil
			default:
				panic(errors.Errorf("sqldrv: invalid right-operand value/type in %v: %#v", expr.r, r))
			}

		default:
			panic(errors.Errorf("sqldrv: invalid left-operand value/type in %v: %#v", expr.l, l))
		}
	...
	case opLT:
		...
		case idealUint:
			...
			case idealUint:
				return l < r, nil
			default:
				panic(errors.Errorf("sqldrv: invalid right-operand value/type in %v: %#v %T", expr.r, r, r))
			}
		case idealInt:
			switch r := r.(type) {
			case idealInt:
...
func (expr *identExpr) eval(ectx *execCtx, vctx map[interface{}]interface{}) (r interface{}, err error) {
	r, ok := vctx[expr.name]
	if !ok {
		err = errors.Errorf("unknown field %q", expr.name)
	}
	return r, err
}
...
func newValueExpr(expr *sqlparser.SQLVal, args []driver.Value) (expression, error) {
	...
	case sqlparser.ValArg:
		if !strings.HasPrefix(s, ":v") {
			return nil, errors.Errorf("rsqldrv: invalid ValArg name %q", s)
		}
	...
	default:
		panic(errors.Errorf("invalid SQLVal type %#v (%T)", expr, expr))
	}
}
...
func idealValArgFrom(v interface{}) interface{} {
	...
	panic(errors.Errorf("rsqldrv: invalid ValArg type %#v", v))
}
