...
// holds necessary information for the
// delaunay triangulation
type Delaunay struct {
	triangles []*Triangle // all triangles created
...
func (d *Delaunay) GetTriangles() []*Triangle {
	...
	rt := make([]*Triangle, 0, len(d.root.A.adjacentTriangles)+len(d.root.B.adjacentTriangles)+len(d.root.C.adjacentTriangles))
	rt = append(rt, d.root.A.adjacentTriangles[:]...)
	rt = append(rt, d.root.B.adjacentTriangles[:]...)
	rt = append(rt, d.root.C.adjacentTriangles[:]...)
	return finalize(d.triangles, rt[:]...)
}

func (d *Delaunay) InsertPoint(p *Point) {
	p.adjacentTriangles = make([]*Triangle, 0)
...
func (d *Delaunay) RemovePoint(p *Point) {
	// if that's the case either an error has occurred or the point is outside or on the root triangle
	if len(p.adjacentTriangles) < 3 {
		// should only happen when user makes a mistake
		panic(fmt.Errorf("delaunay: can't remove point P%s", p))
	}
	...
	// find points on polygon around the point
	...
	for i := 0; j < len(points)-1; {
		if i >= len(p.adjacentTriangles) {
			// should only happen when user makes a mistake
			panic(fmt.Errorf("delaunay: internal error with adjacent triangles for P%s", p))
		}
...
func (d *Delaunay) removeP(points []*Point, parents []*Triangle) {
	...
	if len(points) == 3 {
		...
		points[0].adjacentTriangles = appendT(points[0].adjacentTriangles, nt)
		points[1].adjacentTriangles = appendT(points[1].adjacentTriangles, nt)
		points[2].adjacentTriangles = appendT(points[2].adjacentTriangles, nt)
		...
		*/
		...
		for _, t := range ts {
			...
			if inPolygon(a, b, c) {
				...
				points[a].adjacentTriangles = appendT(points[a].adjacentTriangles, tr)
				points[b].adjacentTriangles = appendT(points[b].adjacentTriangles, tr)
				points[c].adjacentTriangles = appendT(points[c].adjacentTriangles, tr)
...
// if points are ordered clockwise they are outside the polygon
func inPolygon(a, b, c int) bool {
	if c > b {
		if b > a {
			return true
		} else if a > c {
...
// goes down the hierarchy to find the triangle in which the point is located.
// it returns true if the point is on an edge
func findTriangle(t *Triangle, p *Point) (*Triangle, bool) {
	inside, edge := t.inTriangle(p)
	if !inside {
		return nil, false
	}
	if len(t.children) == 0 {
...
func (d *Delaunay) insertP(t *Triangle, p *Point) {
	...
	p.adjacentTriangles = appendT(p.adjacentTriangles, t1, t2, t3)
	t.A.adjacentTriangles = remove(t.A.adjacentTriangles, t)
	t.B.adjacentTriangles = remove(t.B.adjacentTriangles, t)
	t.C.adjacentTriangles = remove(t.C.adjacentTriangles, t)
	t.A.adjacentTriangles = appendT(t.A.adjacentTriangles, t1, t3)
	t.B.adjacentTriangles = appendT(t.B.adjacentTriangles, t1, t2)
	t.C.adjacentTriangles = appendT(t.C.adjacentTriangles, t2, t3)
...
func (d *Delaunay) insertPonE(t1 *Triangle, p *Point) {
	...
	if !t1.A.Equals(t2.A) && !t1.A.Equals(t2.B) && !t1.A.Equals(t2.C) {
		...
		pO1 = t1.B
		pO2 = t1.C
	} else if !t1.B.Equals(t2.A) && !t1.B.Equals(t2.B) && !t1.B.Equals(t2.C) {
		p1 = t1.B
		pO1 = t1.A
		pO2 = t1.C
	} else if !t1.C.Equals(t2.A) && !t1.C.Equals(t2.B) && !t1.C.Equals(t2.C) {
		p1 = t1.C
		pO1 = t1.B
		pO2 = t1.A
	}
	if !t2.A.Equals(t1.A) && !t2.A.Equals(t1.B) && !t2.A.Equals(t1.C) {
		p2 = t2.A
	} else if !t2.B.Equals(t1.A) && !t2.B.Equals(t1.B) && !t2.B.Equals(t1.C) {
		p2 = t2.B
	} else if !t2.C.Equals(t1.A) && !t2.C.Equals(t1.B) && !t2.C.Equals(t1.C) {
		p2 = t2.C
	}
	...
	// adjust the adjacent triangles for all points involved
	p.adjacentTriangles = appendT(p.adjacentTriangles, nt1, nt2, nt3, nt4)
	p1.adjacentTriangles = remove(p1.adjacentTriangles, t1)
	p2.adjacentTriangles = remove(p2.adjacentTriangles, t2)
	pO1.adjacentTriangles = remove(pO1.adjacentTriangles, t1, t2)
	pO2.adjacentTriangles = remove(pO2.adjacentTriangles, t1, t2)
	p1.adjacentTriangles = appendT(p1.adjacentTriangles, nt1, nt3)
	p2.adjacentTriangles = appendT(p2.adjacentTriangles, nt2, nt4)
	pO1.adjacentTriangles = appendT(pO1.adjacentTriangles, nt1, nt2)
	pO2.adjacentTriangles = appendT(pO2.adjacentTriangles, nt3, nt4)
...
// turns triangle into a valid delaunay triangle
...
// flip edge between two triangles
func (d *Delaunay) flip(t1 *Triangle, t2 *Triangle) (*Triangle, *Triangle) {
	...
	pO1.adjacentTriangles = remove(pO1.adjacentTriangles, t1, t2)
	pO2.adjacentTriangles = remove(pO2.adjacentTriangles, t1, t2)
	p1.adjacentTriangles = appendT(p1.adjacentTriangles, nt1, nt2)
	p2.adjacentTriangles = appendT(p2.adjacentTriangles, nt1, nt2)
	pO1.adjacentTriangles = appendT(pO1.adjacentTriangles, nt1)
	pO2.adjacentTriangles = appendT(pO2.adjacentTriangles, nt2)
